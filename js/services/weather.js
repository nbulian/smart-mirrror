app.service('WeatherService', function($http) {
    ...
    this.init = function () {
        return $http.jsonp('https://api.forecast.io/forecast/' + config.forecast.key + '/' +
            config.geoPosition.latitude + ',' + config.geoPosition.longitude + '?units=' +
            config.forecast.units + "&lang=" + config.language + "&callback=JSON_CALLBACK")
            .then(function (response) {
                return service.forecast = response;
            });
    }

    this.minutelyForecast = function () {
