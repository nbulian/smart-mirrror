app.service('TrafficService', function($http, $q, TimeboxService, DebugService) {
    var BING_MAPS = "https://dev.virtualearth.net/REST/V1/Routes/"

    this.getDurationForTrips = function () {
    ...
    function getTripDuration(trip) {
        ...
        $http.get(getEndpoint(trip)).then(function (response) {
            ...
            if (error.status === 404) {
                DebugService.initDebug('No transit information available between start and end');
                DebugService.initDebug('Unavailable');
            } else if (error.status === 401) {
                DebugService.initDebug('Unauthorized. Check your traffic key.');
                deferred.reject('Unauthorized');
            } else {
                DebugService.initDebug('Traffic error', error.statusText);
